...
Runner that casts votes on behalf of users who authorized their agent signer as
...
The sequencer allows one owner per alias, so every person gets a signer of
their own: a [Coinbase CDP](https://docs.cdp.coinbase.com/) server account,
minted the first time they load their contexts and kept in the `signers` table
as `(address, name, signer)`. The UI shows that signer address and asks the
person to authorize it as an alias. An account policy rejects transactions and
plain messages, so the key can only sign typed data such as votes.
...
| `plan`    | Reads active proposals in the watched spaces and the people with a context whose signer is still an authorized alias, drops voters who already voted, and inserts one job per remaining pair |
...
| Route               | What it does                                                                  |
| ------------------- | ----------------------------------------------------------------------------- |
| `POST /context/get` | Returns that person's signer and every context they saved, for all spaces     |
...
`GET /` reports the name, version, watched spaces and dry run state. The UI
reads it to know which spaces the agent votes in.

## Configuration

See `.env.example`. The runner refuses to start without the CDP credentials
(`CDP_API_KEY_ID`, `CDP_API_KEY_SECRET`, `CDP_WALLET_SECRET`), without
`OPENROUTER_API_KEY`, or without at least one space in `SPACES`.
