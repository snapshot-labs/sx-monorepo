...
The AI chains tools automatically: `snapshot-hub-query` to discover data, then `snapshot-vote`, `snapshot-propose`, or `snapshot-follow` to act. Read actions run instantly. Write actions ask you to authorize once (see [Authentication](#authentication)).
...
The server exposes 8 tools. The AI selects them automatically based on your prompt. Reads default to the offchain hub. The AI uses the `snapshot-api-*` tools only when you explicitly ask about onchain spaces (Snapshot X or Governor).

| Tool | Description | Auth |
|---|---|---|
| `snapshot-hub-query` | Run any GraphQL query against the Snapshot hub API (offchain spaces, the default). Auto-binds your address as `$user`. | No |
| `snapshot-hub-schema` | Return the hub GraphQL schema (query fields and entity types). Used when a query fails on an unknown field. | No |
| `snapshot-api-query` | Run any GraphQL query against the Snapshot API (onchain Snapshot X and Governor spaces). Auto-binds your address as `$user`. | No |
| `snapshot-api-schema` | Return the Snapshot API GraphQL schema (query fields and entity types). Used when a query fails on an unknown field. | No |
...
3. **Read.** The hub tools proxy to Snapshot Hub, and the api tools proxy to the Snapshot API. No wallet needed.
