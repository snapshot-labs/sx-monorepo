...
| `snapshot-hub-schema` | Return the hub GraphQL schema. Used when a query fails on an unknown field. | No |
...
| `snapshot-api-schema` | Return the Snapshot API GraphQL schema. Used when a query fails on an unknown field. | No |
