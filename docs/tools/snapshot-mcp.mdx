...
ChatGPT has no install deep link yet, so add the server by hand:
...
Revoke the alias from **Settings > Aliases** on Snapshot. This immediately stops the server from acting on your behalf. See [Authentication](#authentication) for a walkthrough.
</Accordion>
