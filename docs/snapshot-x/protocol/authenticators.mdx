...
* `data`: The ABI encoded transaction payload for the action. Refer to the [Space actions](space-actions) section for more information on the payload contents. 
...
<Info>
Please note, that if you wanted to add sybil resistance to your governance process, this should not be handled by Authenticators, but by [Proposal validations](proposal-validations) or [Voting strategies](voting-strategies).
</Info>
