...
This is what makes integrations like the [MCP server](/tools/mcp) possible: an AI assistant can act on your behalf without ever having access to your private key.
...
<Tip>
If you are using the [MCP server](/tools/mcp), the alias is created and authorized automatically through an OAuth flow the first time the AI calls a write tool.
</Tip>
...
<Warning>
If you are using the [MCP server](/tools/mcp), disconnecting the connector in your AI client stops the AI from making requests but does **not** revoke the alias. To fully revoke, use one of the methods above.
</Warning>
