...
export interface Strategy {
    type: string;
    getParams(call: 'propose' | 'vote', signerAddress: string, address: string, index: number, metadata: Record<string, any> | null, envelope: Envelope<Message>, clientConfig: ClientConfig): Promise<string[]>;
    getExtraProposeCalls(address: string, index: number, envelope: Envelope<Message>, clientConfig: ClientConfig): Promise<Call[]>;
    getVotingPower: (strategyAddress: string, voterAddress: string, metadata: Record<string, any> | null, timestamp: number | null, params: string[], clientConfig: ClientConfig) => Promise<bigint>;
}
...
export type EIP712UpdateProposalMessage = Omit<StarknetEIP712UpdateProposalMessage, 'proposalId'> & {
    authenticator: string;
    proposalId: string;
};
...
export type StrategiesAddresses = {
    index: number;
    address: string;
}[];
